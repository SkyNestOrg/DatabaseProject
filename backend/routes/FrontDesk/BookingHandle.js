...
router.post("/add", authenticateToken, async (req, res) => {
  ...
  console.log("Received data:", { guestId, roomNumber, bookingDate, checkInDate, checkoutDate });
  ...
  try {
    ...
    // DEBUG: Check the specific room we're looking for
    const [specificRoom] = await connection.query(
      `SELECT r.room_number, r.current_status, r.room_type, r.branch_id 
       FROM Room r 
       WHERE r.room_number = ? AND r.branch_id = ?`,
      [roomNumber, branchId]
    );
    console.log("Specific room query result:", specificRoom);
    ...
    // 2️⃣ Validate room (original query)
    console.log("Validating room availability...");
    const [roomResult] = await connection.query(
      `SELECT r.room_number, r.room_type, rt.base_price as price 
       FROM Room r
       JOIN RoomType rt ON r.room_type = rt.type_name
    ...
    console.log("Room availability query result:", roomResult);
    ...
    if (roomResult.length === 0) {
      ...
      if (roomExists.length === 0) {
        return res.status(404).json({ 
          success: false, 
          message: `Room ${roomNumber} does not exist in branch ${branchId}` 
        });
      } else {
        return res.status(400).json({ 
          success: false, 
          message: `Room ${roomNumber} exists but is not available. Current status: ${roomExists[0].current_status}` 
        });
      }
    }
    const roomPrice = roomResult[0].price;
    ...
    // 5️⃣ Insert Booked_Room
    console.log("Inserting booked room...");
    await connection.query(
      `INSERT INTO Booked_Room (room_number, booking_id, branch_id, check_in, check_out, status)
       VALUES (?, ?, ?, ?, ?, 'Booked')`,
      [roomNumber, bookingId, branchId, checkInDate, checkoutDate]
    ...
    res.status(500).json({ 
      ...
      error: error.sqlMessage // Remove in production
    });
  }
});