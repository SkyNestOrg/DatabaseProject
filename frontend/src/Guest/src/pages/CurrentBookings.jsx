...
const CurrentBookings = () => {
  ...
  useEffect(() => {
    ...
    // ensure spinner keyframes are present (client-only)
...
// Add CSS animation for the spinner safely (do not run at module evaluation time)
function ensureSpinnerKeyframes() {
  ...
  try {
    ...
    if (document.styleSheets && document.styleSheets.length) {
      for (let i = 0; i < document.styleSheets.length; i++) {
        try {
          ...
          // accessing cssRules may throw for cross-origin stylesheets, so guard it
          if (s && s.cssRules) { sheet = s; break; }
        } catch (e) {
          continue;
        }
      }
    ...
    if (!sheet) {
      ...
      styleEl.setAttribute('data-generated', 'current-bookings');
      document.head.appendChild(styleEl);
      sheet = styleEl.sheet;
    }

    const keyframeRule = '@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }';

    let exists = false;
    for (let i = 0; sheet && i < sheet.cssRules.length; i++) {
      try {
        if (sheet.cssRules[i].cssText && sheet.cssRules[i].cssText.includes('@keyframes spin')) { exists = true; break; }
      } catch (e) {
    ...
    // In environments where stylesheet modification is not allowed, fail silently
    console.warn('Could not inject spinner keyframes', err);
  }
}
